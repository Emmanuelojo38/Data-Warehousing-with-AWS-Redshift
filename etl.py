...
def load_staging_tables(cur, conn):
        ...
        for idx, query in enumerate(copy_table_queries):
            ...
            print('No. of rows copied into {}: {}'.format(copy_staging_order[idx], row))
...
def insert_tables(cur, conn):
    ...
    for idx, query in enumerate(insert_table_queries):
        ...
        conn.commit()
        row = cur.rowcount
        print('No. of rows inserted into {}: {}'.format(insert_table_order[idx], row))
...
def main():
    ...
    - Loads staging tables from raw log and song files on redshift database
